...
class MainActivity:Activity(){
 ...
 private fun ui(){
  ...
  web=WebView(this).apply{
   ...
   webChromeClient=object:WebChromeClient(){
    override fun onProgressChanged(v:WebView?,newProgress:Int){super.onProgressChanged(v,newProgress);progress.progress=newProgress;if(newProgress>=95&&!loadTimeout)hideLoading()}
 ...
 private fun save(){getSharedPreferences("macro",0).edit().putString("targetPackage",pkg.text.toString().trim()).putLong("interval",interval.text.toString().toLongOrNull()?.coerceAtLeast(20L)?:100L).putString("webhook",webhook.text.toString().trim()).putString("points",points.joinToString(";"){"\${p.x},\${p.y},\${p.delayMs}"}).apply();updateWebhookStatus()}
