...
class MacroAccessibilityService:AccessibilityService(){
 ...
 fun startMacro(){loadConfig();if(points.isEmpty()||targetPackage.isBlank()){notifyWebhook("macro_start_rejected",true);return};running=true;index=0;total=0;session=UUID.randomUUID().toString();updateIndicator(true);notifyWebhook("macro_started");scheduleNext(0)}
 ...
 fun testWebhook(){loadConfig();notifyWebhook("webhook_test",true)}
 fun beginCoordinateCapture(){
  if(capture!=null)return
  val v=FrameLayout(this).apply{setBackgroundColor(Color.argb(40,0,0,0));isClickable=true;addView(TextView(this@MacroAccessibilityService).apply{text="원하는 위치를 터치하세요";textSize=16f;setTextColor(Color.WHITE);setBackgroundColor(Color.argb(190,0,0,0));setPadding(24,18,24,18)});setOnTouchListener{_,e->if(e.action==MotionEvent.ACTION_UP){val x=e.rawX.toInt();val y=e.rawY.toInt();getSharedPreferences("macro",0).edit().putInt("captureX",x).putInt("captureY",y).putBoolean("captureReady",true).apply();removeCapture()};true}}
 ...
 private fun showIndicator(){
  ...
  val v=TextView(this).apply{text="Ⅱ";textSize=12f;gravity=Gravity.CENTER;setTextColor(Color.WHITE);setBackgroundColor(Color.argb(180,100,100,100));contentDescription="MacroV1 실행 상태"}
  val size=(24*resources.displayMetrics.density).toInt();val lp=WindowManager.LayoutParams(size,size,WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY,WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE or WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE,PixelFormat.TRANSLUCENT).apply{gravity=Gravity.TOP or Gravity.START;x=4;y=4}
  try{getSystemService(WindowManager::class.java)?.addView(v,lp);overlay=v}catch(_:Exception){overlay=null}
 }
 ...
 private fun notifyWebhook(event:String,error:Boolean=false){
  val p=getSharedPreferences("macro",Context.MODE_PRIVATE)
  ...
  network.execute{
   for(attempt in 0..2){
    try{
     val c=(URL(u).openConnection()as HttpURLConnection).apply{requestMethod="POST";connectTimeout=2500;readTimeout=2500;doOutput=true;setRequestProperty("Content-Type","application/json; charset=utf-8");setRequestProperty("User-Agent","MacroV1/1.2")}
     c.outputStream.use{it.write(payload.toByteArray(StandardCharsets.UTF_8))};val code=c.responseCode;c.disconnect();if(code in 200..299)break
    }catch(_:Exception){}
    try{Thread.sleep((250L shl attempt).coerceAtMost(1000L))}catch(_:InterruptedException){Thread.currentThread().interrupt();break}
   }
  }
 }
